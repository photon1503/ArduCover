...
//
// ASCOM CoverCalibrator hardware class for ArduCover1
//
    ...
    internal static class CoverCalibratorHardware
    {
        ...
        static CoverCalibratorHardware()
        {
            try
            {
                ...
                tl = new TraceLogger("", "ArduCover1.Hardware");
            ...
            if (newState) // We are connecting
            {
                ...
                else // Instance not already connected, so connect it
                {
                    ...
                    if (uniqueIds.Count == 0) // This is the first connection to the hardware so initiate the hardware connection
                    {
                        ...
                        SharedResources.SharedSerial.PortName = comPort; // Set the COM port name
                        SharedResources.SharedSerial.Connected = true; // Connect to the hardware
            ...
            else // We are disconnecting
            {
                ...
                else // Instance currently connected so disconnect it
                {
                    ...
                    if (uniqueIds.Count == 0) // There are no connected driver instances so disconnect from the hardware
                    {
                        ...
                        SharedResources.SharedSerial.Connected = false; // Disconnect from the hardware
                        //
                    }
        ...
        internal static CoverStatus CoverState
        {
            get
            {
                string response = SharedResources.SendMessage("P"); // Send the command to get the calibrator state

                switch (response)
                {
                    case "1":
                        return CoverStatus.Closed; // 0 = closed
                    case "3":
                        return CoverStatus.Open; // 1 = open
                    case "2":
                        return CoverStatus.Moving; // 2 = moving
                    case "5":
                        return CoverStatus.Error; // 3 = error
                    default:
                        return CoverStatus.Unknown; // 4 = unknown or any other value
                }
            }
        }
        ...
        {
            SharedResources.SendMessage("O"); // Send the command to open the cover
        }
