...
namespace ASCOM.LocalServer
{
    ...
    public static class SharedResources
    {
        ...
        public static string SendMessage(string message)
        {
            ...
            lock (lockObject)
            {
                // Ensure message is framed with start and end markers
                string framedMessage = $"{startMarker}{message}{endMarker}";
                SharedSerial.Transmit(framedMessage);

                string reply = SharedSerial.ReceiveTerminated(">"); // Wait for a reply terminated by '#'
                // extract message from reply between markers
                if (reply.StartsWith(startMarker.ToString()) && reply.EndsWith(endMarker.ToString()))
                {
                    return reply.Substring(1, reply.Length - 2); // Remove start and end markers
                }
                else
                {
                    throw new ASCOM.InvalidOperationException("Received message is not properly framed.");
                }
            }
        }
