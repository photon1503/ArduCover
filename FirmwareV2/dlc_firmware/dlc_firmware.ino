...
#define SAVED_POSITION 13    // EEPROM index for last known position
...
// get saved values from EEPROM
  ...
  if (currentCoverState <= 0)
    currentCoverState = 4; // set cover to 4:Unknown if no recorded state exists
  // Restore timeToMoveCover from EEPROM
  uint32_t savedTime = EEPROMwl.get(SAVED_TIMETOMOVECOVER, timeToMoveCover);
  if (savedTime >= 1000 && savedTime <= 30000)
    timeToMoveCover = savedTime;
  ...
  if (savedOpen >= 0.0f && savedOpen <= 180.0f)
    primaryServoOpenCoverAngle = savedOpen;
  if (savedClose >= 0.0f && savedClose <= 180.0f)
    primaryServoCloseCoverAngle = savedClose;
#endif

#endif

  // read last known position from EEPROM
  primaryServoLastPosition = EEPROMwl.get(SAVED_POSITION, primaryServoLastPosition);

  if (primaryServoLastPosition < 0.0f || primaryServoLastPosition > 180.0f)
  {
    primaryServoLastPosition = primaryServoCloseCoverAngle; // default to closed if invalid
    primaryServo.write(primaryServoOpenCoverAngle);
  }  else
  {
    primaryServo.write(primaryServoLastPosition); // move to last known position
  }
...
void processCommand()
{
  ...
  case 'G':
  {
    ...
    char posStr[8];
    dtostrf(primaryServoLastPosition, 6, 2, posStr);
    snprintf(response, maxNumSendChars, "position:%s", posStr);
...
void haltCover()
{
  ...
  if (currentCoverState == 2)
  {
    ...
    // Save current position to primaryServoLastPosition
    primaryServoLastPosition = primaryServo.read();
// store to SAVED_POSITION in EEPROM
#ifdef ENABLE_SAVING_TO_MEMORY
    EEPROMwl.put(SAVED_POSITION, primaryServoLastPosition);
...
void setMovement()
{
  ...
  if (totalDistance < 1.0f)
    totalDistance = 1.0f; // avoid divide by zero
  proportionalMoveDuration = (uint32_t)(timeToMoveCover * (angleDistance / totalDistance));
  if (proportionalMoveDuration < 100)
    proportionalMoveDuration = 100; // minimum duration for short moves
  attachServo();
  currentCoverState = 2;
